...
warn('Big PR') if git.lines_of_code > ENV['SDM_DANGER_BIG_PR_LINES']
...
warn('Changes to build files') if didModify(ENV['SDM_DANGER_BUILD_FILES'])

# Warn when changing the requirements files
warn('Changes to installation requirements files') if didModify(ENV['SDM_DANGER_INSTALL_REQUIREMENTS_FILES'])

# Fail if changes to immutable files, such as License or CoC
fail('Do not modify the license or Code of Conduct') if didModify(ENV['SDM_DANGER_IMMUTABLE_FILES'])
...
build_artifacts = ENV['SDM_DANGER_REPORT_CIRCLE_CI_ARTIFACTS']
if build_artifacts
